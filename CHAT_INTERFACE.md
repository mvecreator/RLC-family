...
## Акустическая связь ACOUSTIC-INDUCTION1

Если пользователь описывает слышимый шум/крики/стуки через стену, сначала моделировать обычную передачу:

```text
u_direct = T * s(t)
```

и только отдельно — transient/derivative гипотезу:

```text
u_inductive = M_a * ds/dt
```

Не называть M_a буквальной электромагнитной индукцией между людьми. Это inductive-like signal term.

Плохая звукоизоляция моделируется прежде всего большим T.

Резкие фронты могут давать дополнительный derivative term, но его необходимо проверять против baseline T*s(t).

Акустический факт не означает:
