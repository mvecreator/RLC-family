...
## 9. Исчезновение второго сна

Историческое наблюдение уточнено: двухфазность была не конечным стабильным состоянием.
...
-> второй сон начинает растягиваться/отодвигаться во времени
-> его выраженность постепенно уменьшается
-> снова остаётся один основной сон
```

Поэтому sleep-layer теперь имеет как минимум два независимых состояния:

```text
split_strength(t)
secondary_bout_weight(t)
```

и отдельный:

```text
secondary_temporal_stretch_index(t)
```

Точная поздняя длительность промежутка неизвестна и не выдумывается.

Эта траектория реализована в `simulator/sleep_trajectory_model.py` и должна использоваться как qualitative outcome в OUTCOME-BLIND1.

---

## 10. CAFFEINE1

[CAFFEINE1](CAFFEINE1.md) отделяет кофе от generic forcing.

Считается временной caffeine burden с настраиваемым half-life и временем приёма относительно сна.

Ключевая граница:

```text
caffeine burden = feature
sleep resolution = observed outcome
causal relation = not assumed
```

Именно OUTCOME-BLIND1 должен проверить, добавляет ли caffeine burden что-либо к предсказанию будущей траектории сна.
